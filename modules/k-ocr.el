...
;;; Interactive Commands - Search & Navigation

;;;###autoload
(defun k-ocr-search-window ()
  "OCR current window and search interactively within the text."
  (interactive)
  (let ((file (expand-file-name "ocr-search.png" k-ocr-temp-dir)))
    (k-ocr--screenshot-window file)
    (let ((text (k-ocr--extract-text file)))
      (with-current-buffer (get-buffer-create "*Window Text*")
        (erase-buffer)
        (insert text)
        (goto-char (point-min))
        (pop-to-buffer (current-buffer))
              ...
              (let ((text (nth 11 fields))
                ...
                (when (and text 
                  ...
                  (let ((x (+ left (/ width 2)))
                        (y (+ top (/ height 2))))
                    (setq found t)
                    (message "Found '%s' at (%d, %d), clicking..." text x y)
...
;;; Interactive Commands - URL & Link Extraction
...
;;;###autoload
(defun k-ocr-urls-to-org ()
  "OCR a region, extract URLs, format as org links."
  (interactive)
  (let ((file (expand-file-name "ocr-urls.png" k-ocr-temp-dir)))
    (message "Select region...")
    (k-ocr--screenshot-region file)
    (let* ((text (k-ocr--extract-text file))
      ...
      (with-temp-buffer
        ...
        (while (re-search-forward "https?://[^\s\n\t\"'<>]+" nil t)
          (push (match-string 0) urls)))
      (if urls
          (with-current-buffer (get-buffer-create "*OCR URLs*")
            (erase-buffer)
            (org-mode)
...
(defun k-ocr-translate (target-lang)
  "OCR a region and translate to TARGET-LANG."
...
;;; Interactive Commands - Monitoring

;;;###autoload
(defun k-ocr-watch-region (interval callback)
  "Watch a screen region every INTERVAL seconds, call CALLBACK on change.
CALLBACK receives the new text as argument."
  ...
  (let ((file (expand-file-name "ocr-watch.png" k-ocr-temp-dir))
        (geometry-file (expand-file-name "ocr-watch-geom.txt" k-ocr-temp-dir))
        (last-text ""))
    (message "Select region to watch...")
    ;; Get geometry from maim
    (shell-command (format "%s -s %s" k-ocr-screenshot-command file))
    (setq k-ocr--watch-timer
          (run-with-timer 
           0 interval
           (lambda ()
             (when (file-exists-p file)
               (let ((text (k-ocr--extract-text file)))
                 (unless (string= text last-text)
                   (setq last-text text)
                   (funcall callback text))
                 ;; Take new screenshot for next iteration
                 (shell-command (format "%s -s %s 2>/dev/null || true" 
                                        k-ocr-screenshot-command file)))))))))
...
;;;###autoload
(defun k-ocr-watch-alert ()
  "Watch a region and show alert when text changes."
  (interactive)
  (k-ocr-watch-region 
   3
   (lambda (text)
     (message "Region changed: %s" (truncate-string-to-width text 50)))))
...
;;;###autoload
(defun k-ocr-window-to-org ()
  "OCR current EXWM window and create org entry with screenshot."
  (interactive)
  (let* ((timestamp (format-time-string "%Y%m%d-%H%M%S"))
         (img-file (expand-file-name (format "ocr-%s.png" timestamp)
                                     "~/org/images/"))
         (text-file (expand-file-name "ocr-temp.png" k-ocr-temp-dir)))
    (make-directory (file-name-directory img-file) t)
    (k-ocr--screenshot-window text-file)
    (copy-file text-file img-file t)
    (let ((text (k-ocr--extract-text text-file))
          (title (or (and (boundp 'exwm-title) exwm-title) "Screenshot")))
      (with-current-buffer (get-buffer-create "*OCR Org Entry*")
        (erase-buffer)
        (org-mode)
        (insert (format "* %s\n" title))
